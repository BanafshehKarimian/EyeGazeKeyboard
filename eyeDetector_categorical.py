...
def LeNet(train_data, train_labels,test_data, test_labels,inps,d):
    ...
    model.save('my_model_epoch_10_all.h5')
...
def dataset_seperator(img,label,size): # SEPERATE TO TRAIN AND TEST
    dist = []
    for i in range(size):
    ...
    X_train, X_test, y_train, y_test = train_test_split(img, label2, test_size=0.2)# SPLIT DATA TEST SIZE = 20%
    return X_train, X_test, to_categorical(y_train), to_categorical(y_test),cord
...
print(LeNet(np.array(train), np.array(train_l), np.array(test), np.array(test_l), (100, 100, 3), len(cord)))
